...
class MyServer(socketserver.BaseRequestHandler):
    def handle(self):
            ...
            while True:
                ...
                # FTP文件续传
                elif client_dict["header"] == "send_file_again":
                    ObjFtpAgain = FtpServerAgain(conn)
                    dt_again = ObjFtp.recv_write()
                    ObjFtpAgain.rename(dt_again)
                    ObjFtpAgain.sendreport(dt_again)
...
class FtpServerAgain:
    def __init__(self,conn):
        self.Conn = conn
    def recv_write(self):
        # 返回一个包给客户端（解决粘包）
        self.Conn.sendall(bytes("send_respone_again_Hander", encoding='utf-8'))
        # 接收
        file_size = 0
        dict_bytes = self.Conn.recv(1024)
        dict_str = str(dict_bytes, encoding='utf-8')
        ...
        file_size = int(dt['size'])
        #print(dt)
        self.Conn.sendall(bytes("send_respone_again_FileInfo", encoding='utf-8'))
